...
"""
Example top-bottom code start
"""
topbottomBoolean = False #When this boolean is true, the applied potential diff will be top-bottom
#When it is false the applied potential will be left-right
...
sorted_vertices = sorted(enumerate(vertpos), key=lambda x: x[-1][0])#x[-1][-1])  # Sort by y-coordinate #x[-1][0])  # Sort by x-coordinate
if topbottomBoolean:
        sorted_vertices = sorted(enumerate(vertpos), key=lambda x: x[-1][-1])
